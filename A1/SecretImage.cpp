...
SecretImage::SecretImage(const GrayscaleImage& image) : width(image.get_width()), height(image.get_height()) {
    ...
    for(int i = 0; i < height; i++) {
        for(int j = 0; j < width; j++) {
            if(i <= j) {
                upper_triangular[i * width - (i * (i - 1)) / 2 + j - i] = image.get_pixel(i, j);
            } else {
...
SecretImage::SecretImage(int w, int h, int * upper, int * lower): width(w), height(h), upper_triangular(upper), lower_triangular(lower) {
    ...
    int lowerSize = (h * (h - 1)) / 2;

    // Step 2: Copy the data from the passed-in upper and lower arrays to the instance variables
...
void SecretImage::save_back(const GrayscaleImage& image) {
    ...
    for(int i = 0; i < height; i++) {
        for(int j = 0; j < width; j++) {
            if(i <= j) {
                int index = i * width - i * (i - 1) / 2 + j - i;
                upper_triangular[index] = image.get_pixel(i, j);
            } else {
                int index = i * (i + 1) / 2 + j;
                lower_triangular[index] = image.get_pixel(i, j);
            }
        }
    }
...
// Returns the "hei"ght of the secret image.
