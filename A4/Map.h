...
struct MapNode
{
    ...
    ~MapNode()
    {
        delete isle; // Free the dynamically allocated Isle
    }
};
...
private:
    ...
    bool initilized = false; // Flag for initilization
    int rebalanceCount = 0; // Counter for rebalancing
    // Height of a node
    int height(MapNode *node);
    int countgolds = 0;
    int counteinsteins = 0;
