...
protected:
    ...
    {
        if (!isValidIndex(index) || realmShapers[index] == nullptr)
            return;
        inOrderHelper(2 * index + 1, result); // Left child
        result.push_back(realmShapers[index]->getName());
        inOrderHelper(2 * index + 2, result); // Right child
    }
    ...
    void preOrderHelper(int index, std::vector<std::string> &result)
    {
        if (!isValidIndex(index) || realmShapers[index] == nullptr)
            return;
        result.push_back(realmShapers[index]->getName());
        preOrderHelper(2 * index + 1, result); // Left child
        preOrderHelper(2 * index + 2, result); // Right child
    }
    void postOrderHelper(int index, std::vector<std::string> &result)
    {
        if (!isValidIndex(index) || realmShapers[index] == nullptr)
            return;
        postOrderHelper(2 * index + 1, result); // Left child
        postOrderHelper(2 * index + 2, result); // Right child
        result.push_back(realmShapers[index]->getName());
    }
